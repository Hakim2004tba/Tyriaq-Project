-- Tyriaq — per-project access, and the fix for the notify_user clash.
--
-- Run this in the Supabase SQL editor: select all, press Run.
-- It finishes with "Success. No rows returned".
--
-- Two things are in here:
--   1. Dropping the older notify_user, which was left behind when the
--      space-join migration added an argument to it. Two functions
--      matching the same call is why sending a message failed with
--      "function public.notify_user(...) is not unique".
--   2. Per-project levels: letting somebody into a space can now say
--      which projects they edit and which they only read.
    ...
    loop
      if entry.wanted in ('viewer', 'commenter', 'editor', 'admin') then
        ...
        values (
          entry.project_id,
          request.user_id,
          -- The old role column stays meaningful: a viewer is a viewer
          -- on the roster too, so anything reading `role` alone still
          -- sees something true.
          case when entry.wanted = 'viewer' then 'viewer' else 'member' end,
...
begin
  ...
  values (
    p_project, p_user,
    case when p_level = 'viewer' then 'viewer' else 'member' end,
    p_level
  )
  on conflict (project_id, user_id)
  do update set level = excluded.level, role = excluded.role;
