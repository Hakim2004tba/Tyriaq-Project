-- Tyriaq — plans, subscriptions and limits
--
-- The Upgrade button has been decoration: the admin panel reads sample
-- data, no workspace has a plan, and nothing anywhere is limited. Which
-- means Tyriaq cannot take money today, and — more quietly — that there
-- is no shape for a paid tier to exist in when it can.
--
-- Three tables, and deliberately no payment provider in any of them.
-- Stripe does not operate in Algeria, so which provider this ends up on
-- is an open question; `provider` and `provider_ref` are two text
-- columns that any of them can fill, and everything else here works
-- without one. A workspace can be put on a plan by hand today and the
-- product will honour it.
