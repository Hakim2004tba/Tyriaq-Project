-- Tyriaq — what the back office needs that the product does not
--
-- Most of the admin panel can be answered from tables that already
-- exist: users, workspaces, projects, tasks, subscriptions. Three of its
-- pages cannot, because they are about running the business rather than
-- doing the work — support, the audit trail, and platform settings.
--
-- Everything here is staff-only. None of it is readable by a customer,
-- with one deliberate exception: somebody can see their OWN support
-- tickets, because a support system the customer cannot read is a
-- suggestion box.
